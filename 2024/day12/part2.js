...
for (let i = 0; i < map.length; i++) {
  for (let j = 0; j < map[i].length; j++) {
    ...
    // console.log(fences.h);
    let sides = 0;
    for (let a = 0; a < fences.h.length; a++) {
      for (let b = 0; b < fences.h[a].length; b++) {
        if (fences.h[a][b]) {
          sides++;
          // console.log(a, b, fences.h[a]);
          while (fences.h[a][b + 1] === fences.h[a][b]) {
            // console.log(">", a, b + 1, fences.h[a]);
            b++;
          }
        }
      }
    ...
    const price = area * sides * 2;
    // console.log(area, " * ", sides * 2, " = ", price);
    sum += price;
...
function getFencesMap() {
  return {
    h: Array(map.length + 1)
      .fill(0)
      .map(() => Array(map[1].length).fill(0)),
    v: Array(map.length)
      .fill(0)
      .map(() => Array(map[1].length + 1).fill(0)),
  };
}
...
function getRegionDimensions(i, j, id, fences = getFencesMap()) {
  ...
  map[i][j] = null;
  fences.h[i][j] = fences.h[i][j] ? 0 : 1;
  fences.h[i + 1][j] = fences.h[i + 1][j] ? 0 : -1;
  fences.v[i][j] = fences.v[i][j] ? 0 : 1;
  fences.v[i][j + 1] = fences.v[i][j + 1] ? 0 : -1;
